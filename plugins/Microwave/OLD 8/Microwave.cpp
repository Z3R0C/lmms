...
Microwave::Microwave( InstrumentTrack * _instrument_track ) :
	...
	m_modScroll( 1, 1, 256, 0.0001f, this, tr( "Mod Scroll" ) ),
	...
	m_visualize( false, this )
{
	...
	for( int i = 0; i < 8; ++i )
	{
		...
		vocoBandNum[i] = new FloatModel( 64, 1, 64, 1, this, tr( "Number of Bands" ) );
		vocoFreqBias[i] = new FloatModel( 0, -1, 1, 0.0001f, this, tr( "Frequency Bias" ) );
		vocoSwapInputs[i] = new BoolModel( false, this );
		vocoLowBound[i] = new FloatModel( 20, 20, 20000, 1, this, tr( "Low Bound" ) );
		vocoHighBound[i] = new FloatModel( 20000, 0, 20000, 1, this, tr( "High Bound" ) );
		vocoEnabled[i] = new BoolModel( false, this );
		vocoReactTime[i] = new FloatModel( 0.01, 0, 1, 0.00001, this, tr( "Reaction Time" ) );
	...
	for( int i = 0; i < 8; ++i )
	{
		...
		connect( vocoBandNum[i], &FloatModel::dataChanged,
            			this, [this, i]() { valueChanged(51, i); } );
		connect( vocoFreqBias[i], &FloatModel::dataChanged,
            			this, [this, i]() { valueChanged(52, i); } );
		connect( vocoSwapInputs[i], &BoolModel::dataChanged,
            			this, [this, i]() { valueChanged(53, i); } );
		connect( vocoLowBound[i], &FloatModel::dataChanged,
            			this, [this, i]() { valueChanged(54, i); } );
		connect( vocoHighBound[i], &FloatModel::dataChanged,
            			this, [this, i]() { valueChanged(55, i); } );
		connect( vocoEnabled[i], &BoolModel::dataChanged,
            			this, [this, i]() { valueChanged(56, i); } );
		connect( vocoReactTime[i], &FloatModel::dataChanged,
            			this, [this, i]() { valueChanged(58, i); } );
...
void Microwave::valueChanged( int which, int num )
{
	...
	switch( which )
	{
		...
		case 51: vocoBandNumArr[num] = vocoBandNum[num]->value(); break;
		case 52: vocoFreqBiasArr[num] = vocoFreqBias[num]->value(); break;
		case 53: vocoSwapInputsArr[num] = vocoSwapInputs[num]->value(); break;
		case 54: vocoLowBoundArr[num] = vocoLowBound[num]->value(); break;
		case 55: vocoHighBoundArr[num] = vocoHighBound[num]->value(); break;
		case 56: vocoEnabledArr[num] = vocoEnabled[num]->value(); break;
		case 57: m_mutedArr[num] = m_muted[num]->value(); break;
		case 58: vocoReactTimeArr[num] = vocoReactTime[num]->value(); break;
	...
	for( int i = 0; i < nphList.length(); ++i )
	{
		...
		switch( which )
		{
			...
			case 51: ps->vocoBandNum[num] = vocoBandNum[num]->value(); break;
			case 52: ps->vocoFreqBias[num] = vocoFreqBias[num]->value(); break;
			case 53: ps->vocoSwapInputs[num] = vocoSwapInputs[num]->value(); break;
			case 54: ps->vocoLowBound[num] = vocoLowBound[num]->value(); break;
			case 55: ps->vocoHighBound[num] = vocoHighBound[num]->value(); break;
			case 56: ps->vocoEnabled[num] = vocoEnabled[num]->value(); break;
			case 57: ps->muted[num] = m_muted[num]->value(); break;
			case 58: ps->vocoReactTime[num] = vocoReactTime[num]->value(); break;
...
// This will definitely be deleted before release.  If not, go kill Douglas.
	...
	InstrumentView( _instrument, _parent )
{
	...
	for( int i = 0; i < 8; ++i )
	{
		...
		vocoBandNumKnob[i] = new Knob( knobMicrowave, this );
		vocoBandNumKnob[i]->setHintText( tr( "Number of Bands" ), "" );

		vocoFreqBiasKnob[i] = new Knob( knobMicrowave, this );
		vocoFreqBiasKnob[i]->setHintText( tr( "Frequency Bias Knob" ), "" );

		vocoSwapInputsToggle[i] = new LedCheckBox( "Swap Inputs", this, tr( "Swap Inputs" ), LedCheckBox::Green );

		vocoLowBoundKnob[i] = new Knob( knobMicrowave, this );
		vocoLowBoundKnob[i]->setHintText( tr( "Low Bound" ), "" );

		vocoHighBoundKnob[i] = new Knob( knobMicrowave, this );
		vocoHighBoundKnob[i]->setHintText( tr( "High Bound" ), "" );

		vocoEnabledToggle[i] = new LedCheckBox( "Vocoder Enabled", this, tr( "Vocoder Enabled" ), LedCheckBox::Green );

		vocoReactTimeKnob[i] = new Knob( knobMicrowave, this );
		vocoReactTimeKnob[i]->setHintText( tr( "Reaction Time" ), "" );
		...
		m_sampleDetuneKnob[i] = new Knob( knobMicrowave, this );
		m_sampleDetuneKnob[i]->setHintText( tr( "Detune" ), "" );
		m_samplePhaseKnob[i] = new Knob( knobMicrowave, this );
		m_samplePhaseKnob[i]->setHintText( tr( "Phase" ), "" );
		m_samplePhaseRandKnob[i] = new Knob( knobMicrowave, this );
...
void MicrowaveView::modelChanged()
{
	...
	for( int i = 0; i < 8; ++i )
	{
		...
		vocoBandNumKnob[i]->setModel( b->vocoBandNum[i] );
		vocoFreqBiasKnob[i]->setModel( b->vocoFreqBias[i] );
		vocoSwapInputsToggle[i]->setModel( b->vocoSwapInputs[i] );
		vocoLowBoundKnob[i]->setModel( b->vocoLowBound[i] );
		vocoHighBoundKnob[i]->setModel( b->vocoHighBound[i] );
		vocoEnabledToggle[i]->setModel( b->vocoEnabled[i] );
		vocoReactTimeKnob[i]->setModel( b->vocoReactTime[i] );
...
void MicrowaveView::updateScroll()
{
	...
	for( int i = 0; i < 8; ++i )
	{
		...
		vocoBandNumKnob[i]->move( 20 + 1000 - scrollVal, i*30 - effectScrollVal + 480 );
		vocoFreqBiasKnob[i]->move( 50 + 1000 - scrollVal, i*30 - effectScrollVal + 480 );
		vocoSwapInputsToggle[i]->move( 80 + 1000 - scrollVal, i*30 - effectScrollVal + 480 );
		vocoLowBoundKnob[i]->move( 110 + 1000 - scrollVal, i*30 - effectScrollVal + 480 );
		vocoHighBoundKnob[i]->move( 140 + 1000 - scrollVal, i*30 - effectScrollVal + 480 );
		vocoEnabledToggle[i]->move( 170 + 1000 - scrollVal, i*30 - effectScrollVal + 480 );
		vocoReactTimeKnob[i]->move( 200 + 1000 - scrollVal, i*30 - effectScrollVal + 480 );

		m_sampleEnabledToggle[i]->move( 108 + 500 - scrollVal, 200 );
		m_sampleGraphEnabledToggle[i]->move( 128 + 500 - scrollVal, 200 );
		m_sampleMutedToggle[i]->move( 148 + 500 - scrollVal, 200 );
		m_sampleKeytrackingToggle[i]->move( 168 + 500 - scrollVal, 200 );
		m_sampleLoopToggle[i]->move( 188 + 500 - scrollVal, 200 );

		m_sampleVolumeKnob[i]->move( 38 + 500 - scrollVal, 170 );
		m_samplePanningKnob[i]->move( 68 + 500 - scrollVal, 170 );
		m_sampleDetuneKnob[i]->move( 98 + 500 - scrollVal, 170 );
		m_samplePhaseKnob[i]->move( 128 + 500 - scrollVal, 170 );
		m_samplePhaseRandKnob[i]->move( 158 + 500 - scrollVal, 170 );
	}
	...
	sample_rate( _sample_rate )
{
	...
	memcpy( vocoBandNum, _vocoBandNumArr, sizeof(float) * 8 );
	memcpy( vocoFreqBias, _vocoFreqBiasArr, sizeof(float) * 8 );
	memcpy( vocoSwapInputs, _vocoSwapInputsArr, sizeof(float) * 8 );
	memcpy( vocoLowBound, _vocoLowBoundArr, sizeof(float) * 8 );
	memcpy( vocoHighBound, _vocoHighBoundArr, sizeof(float) * 8 );
	memcpy( vocoEnabled, _vocoEnabledArr, sizeof(float) * 8 );
	memcpy( vocoReactTime, _vocoReactTimeArr, sizeof(float) * 8 );
	...
	for( int i=0; i < 8; ++i )
	{
		sample_sampleindex[i] = fmod( fastRandf( samples[i][0].size() ) * ( samplePhaseRand[i] / 100.f ), samples[i][0].size() );
	}
...
std::vector<float> mSynth::nextStringSample( float waveforms[8][262144], float * subs, float * sampGraphs, std::vector<float> (&samples)[8][2], int maxModEnabled, int maxSubEnabled, int maxSampleEnabled, int sample_rate )// "waveforms" here may cause CPU problem, check back later
{
	//=====================//
	//== MAIN OSCILLATOR ==//
	//=====================//
		...
		if( modEnabled[l] )
		{
			...
			switch( m_modIn[l] )
			{
				...
				case 1:
					...
					break;
				case 2:
					curModVal[0] = lastSubVal[m_modInNum[l]-1];
					curModVal[1] = curModVal[0];
					break;
				case 3:
			...
			switch( m_modIn2[l] )
			{
				...
				case 1:
					...
					break;
				case 2:
					curModVal2[0] = lastSubVal[m_modInNum2[l]-1];
					curModVal2[1] = curModVal2[0];
					break;
				case 3:
			...
			// Calculate curve
			curModValCurve[0] = pow( ( curModVal[0] + 1 ) / 2.f, 1.f / ( m_modInCurve[l] / 50.f ) );
			curModValCurve[1] = pow( ( curModVal[1] + 1 ) / 2.f, 1.f / ( m_modInCurve[l] / 50.f ) );
			curModVal2Curve[0] = pow( ( curModVal2[0] + 1 ) / 2.f, 1.f / ( m_modInCurve2[l] / 50.f ) );
			curModVal2Curve[1] = pow( ( curModVal2[1] + 1 ) / 2.f, 1.f / ( m_modInCurve2[l] / 50.f ) );
			...
			switch( m_modOutSec[l] )
			{
				...
				case 1:// Main Oscillator
				{
					switch( m_modOutSig[l] )
					{
						...
						case 3:// Send input to Modify
							_modifyVal[m_modOutSecNum[l]-1] = qMax( 0.f, _modifyVal[m_modOutSecNum[l]-1] + comboModValMono * 1024.f );
				...
				case 6:// Vocoder Input
				{
					vocoOutputs[m_modOutSig[l]][m_modOutSecNum[l]-1][0] = 0;
					vocoOutputs[m_modOutSig[l]][m_modOutSecNum[l]-1][1] = 0;

					Fs = sample_rate;
					
					for( int m = 0; m < vocoBandNum[l]; ++m )
					{
						cutoff = ( vocoLowBound[l] != 1 ) ? ( vocoLowBound[l] + ( ( m / ( vocoBandNum[l] - 1 ) ) * ( vocoHighBound[l] - vocoLowBound[l] ) ) ) : ( ( vocoHighBound[l] - vocoLowBound[l] ) / 2.f );
						reso = vocoBandNum[l] / 8.f;
						w0 = F_2PI * cutoff / Fs;
						...
						// Translation of this monstrosity: y[n] = (b0/a0)*x[n] + (b1/a0)*x[n-1] + (b2/a0)*x[n-2] - (a1/a0)*y[n-1] - (a2/a0)*y[n-2]
						// See www.musicdsp.org/files/Audio-EQ-Cookbook.txt
						// NOTE that some part of this formula has been removed, since they are unnecessary for a bandpass filter if the filter type is never going to change.
						vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][0] = (b0/a0)*curModVal[0] + (b2/a0)*vocoPrevSampIn[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][0] - (a1/a0)*vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][0] - (a2/a0)*vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][0];// Left ear
						vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][1] = (b0/a0)*curModVal[1] + (b2/a0)*vocoPrevSampIn[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][1] - (a1/a0)*vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][1] - (a2/a0)*vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][1];// Right ear

						vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][0] = (b0/a0)*curModVal2[0] + (b2/a0)*vocoPrevSampIn2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][0] - (a1/a0)*vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][0] - (a2/a0)*vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][0];// Left ear
						vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][1] = (b0/a0)*curModVal2[1] + (b2/a0)*vocoPrevSampIn2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][1] - (a1/a0)*vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][1] - (a2/a0)*vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][1];// Right ear

						vocoEnvFollow[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0] = ( vocoEnvFollow[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0] * ( 1.f - vocoReactTime[l] ) ) + ( abs( vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][0] ) * vocoReactTime[l] );
						vocoEnvFollow[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1] = ( vocoEnvFollow[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1] * ( 1.f - vocoReactTime[l] ) ) + ( abs( vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][1] ) * vocoReactTime[l] );

						vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][0] *= vocoEnvFollow[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0];
						vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][1] *= vocoEnvFollow[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1];

						// Send back past samples
						// I'm sorry you had to read this
						vocoPrevSampIn[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][0] = vocoPrevSampIn[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][0];
						vocoPrevSampIn[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][1] = vocoPrevSampIn[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][1];
					
						vocoPrevSampIn[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][0] = curModVal[0];
						vocoPrevSampIn[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][1] = curModVal[1];
					
						vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][0] = vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][0];
						vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][1] = vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][1];
					
						vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][0] = vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][0];
						vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][1] = vocoPrevSampOut[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][1];

						// Send back past samples for the second set of filters
						vocoPrevSampIn2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][0] = vocoPrevSampIn2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][0];
						vocoPrevSampIn2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][1] = vocoPrevSampIn2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][1];
					
						vocoPrevSampIn2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][0] = curModVal2[0];
						vocoPrevSampIn2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][1] = curModVal2[1];
					
						vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][0] = vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][0];
						vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][2][1] = vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][1];
					
						vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][0] = vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][0];
						vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][1][1] = vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][1];

						//Output
						vocoOutputs[m_modOutSig[l]][m_modOutSecNum[l]-1][0] += vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][0];
						vocoOutputs[m_modOutSig[l]][m_modOutSecNum[l]-1][1] += vocoPrevSampOut2[m_modOutSig[l]][m_modOutSecNum[l]-1][m][0][1];
					}
					break;
				}
				case 7:// Sample Oscillator
				{
					switch( m_modOutSig[l] )
					{
						case 0:
	...
	{
		int sampleSize = samples[l][0].size();
		if( sampleEnabled[l] && sample_sampleindex[l] < sampleSize )
		{
			...
			if( sampleLoop[l] )
			{
				sample_sampleindex[l] = fmod( sample_sampleindex[l], samples[l][0].size() );
			}
		}
		else
		{
	...
	for( int l = 0; l < 8; ++l )
	{
		if( vocoEnabled[l] )
		{
			for( int m = 0; m < 8; ++m )
			{
				sampleAvg[0] += vocoOutputs[l][m][0];
				sampleAvg[1] += vocoOutputs[l][m][1];
			}
		}
	}
...
void mSynth::refreshValue( int which, int num )
{
	...
	switch( which )
	{
		...
		case 51: vocoBandNum[num] = mwc->vocoBandNum[num]->value(); break;
		case 52: vocoFreqBias[num] = mwc->vocoFreqBias[num]->value(); break;
		case 53: vocoSwapInputs[num] = mwc->vocoSwapInputs[num]->value(); break;
		case 54: vocoLowBound[num] = mwc->vocoLowBound[num]->value(); break;
		case 55: vocoHighBound[num] = mwc->vocoHighBound[num]->value(); break;
		case 56: vocoEnabled[num] = mwc->vocoEnabled[num]->value(); break;
		case 57: muted[num] = mwc->m_muted[num]->value(); break;
		case 58: vocoReactTime[num] = mwc->vocoReactTime[num]->value(); break;
