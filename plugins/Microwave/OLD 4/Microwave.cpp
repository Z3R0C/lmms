...
std::vector<float> bSynth::nextStringSample( int * modifyModeVal, float * _modifyVal, float * _morphVal, float * rangeVal, float * sprayVal, float * unisonVoices, float * unisonDetune, float * unisonMorph, float * unisonModify, float * morphMaxVal, float * detuneVal, float waveforms[8][262144], float * subs, bool * subEnabled, float * subVol, float * subPhase, float * subPhaseRand, float * subDetune, bool * subMuted, bool * subKeytrack, float * subSampLen, bool * subNoise, int * sample_length, int * m_modIn, int * m_modInNum, int * m_modInOtherNum, float * m_modInAmnt, float * m_modInCurve, int * m_modIn2, int * m_modInNum2, int * m_modInOtherNum2, float * m_modInAmnt2, float * m_modInCurve2, int * m_modOutSec, int * m_modOutSig, int * m_modOutSecNum, float * phase, float * vol, float * filtInVol, int * filtType, int * filtSlope, float * filtCutoff, float * filtReso, float * filtGain, float * filtSatu, float * filtWetDry, float * filtBal, float * filtOutVol, bool * filtEnabled, bool * enabled, bool * modEnabled )// "waveforms" here may cause CPU problem, check back later
{
	...
	for( int i = 0; i < 8; i++ )
	{
		...
		for( int l = 0; l < unisonVoices[i]; l++ )
		{
			...
			{
				case 0:// Pulse Width
					sample_morerealindex[i][l] /= ( -modifyVal[i] + sample_length[i] ) / sample_length[i];
					break;
				case 1:// Stretch Left Half
					if( sample_morerealindex[i][l] < ( sample_length[i] + sample_length_modify[i][l] ) / 2.f )
					{
						sample_morerealindex[i][l] -= (sample_morerealindex[i][l] * (modifyVal[i]/( sample_length[i] )));
					}
					break;
				case 2:// Shrink Right Half To Left Edge
					if( sample_morerealindex[i][l] > ( sample_length[i] + sample_length_modify[i][l] ) / 2.f )
					{
						sample_morerealindex[i][l] += (((sample_morerealindex[i][l])+sample_length[i]/2.f) * (modifyVal[i]/( sample_length[i] )));
					}
					break;
				case 3:// Weird 1
				...
				case 7:// Stretch From Center
					sample_morerealindex[i][l] -= sample_length[i] / 2.f;
					sample_morerealindex[i][l] /= sample_length[i] / 2.f;
			...
			{
				loopStart = qBound(0.f, morphVal[i] - rangeVal[i], 262144.f / sample_length[i] );
				loopEnd = qBound(0.f, morphVal[i] + rangeVal[i], 262144.f / sample_length[i]) + 1;
				...
				currentSampLen = sample_length[i];
			...
			while ( sample_realindex[i][l] >= sample_length[i] + sample_length_modify[i][l] ) {
				sample_realindex[i][l] -= sample_length[i] + sample_length_modify[i][l];
