...
std::vector<float> bSynth::nextStringSample( float waveforms[8][262144], float * subs, float * sampGraphs, int maxModEnabled, int maxSubEnabled )// "waveforms" here may cause CPU problem, check back later
{
	//=====================//
	//== MAIN OSCILLATOR ==//
	//=====================//
				...
				case 5:
				{
					switch( m_modOutSig[l] )
					{
						...
						case 1:// Mod In Amount
							m_modInAmnt[m_modOutSecNum[l]-1] = m_modInAmnt[m_modOutSecNum[l]-1] + ( round((curveModVal)*100.f*0.5f) ) * ( m_modInAmnt[l] / 100.f );
							modValType.push_back( 38 );
							modValNum.push_back( m_modOutSecNum[l]-1 );
							break;
						case 2:// Mod In Curve
							m_modInCurve[m_modOutSecNum[l]-1] = qMax( 0.f, m_modInCurve[m_modOutSecNum[l]-1] + ( round((curveModVal)*100.f*0.5f) ) * ( m_modInAmnt[l] / 100.f ) );
...
Microwave::Microwave( InstrumentTrack * _instrument_track ) :
	...
	m_loadAlg( 0, 0, 4, 1, this, tr( "Wavetable Loading Algorithm" ) ),
						...
						sampleFrame * _working_buffer )
{
	...
	{
		std::vector<float> sample = ps->nextStringSample( waveforms, subs, sampGraphs, maxModEnabled, maxSubEnabled );
	...
	InstrumentView( _instrument, _parent )
{
	...
	sampNumBox = new LcdSpinBox( 1, "microwave", this, "Sample Number" );

	mainNumBox = new LcdSpinBox( 1, "microwave", this, "Oscillator Number" );
