...
/*	delete[] morph;
	...
	delete[] filtFeedback;
	delete[] filtDetune;
	delete[] filtKeytracking;

	delete[] sampleEnabled;
	delete[] sampleGraphEnabled;
	delete[] sampleMuted;
	delete[] sampleKeytracking;
	delete[] sampleLoop;

	delete[] sampleVolume;
	delete[] samplePanning;
	delete[] sampleDetune;
	delete[] samplePhase;
	delete[] samplePhaseRand;
	delete[] sampleStart;
	delete[] sampleEnd;
	...
	delete[] modIn2;
	delete[] modInNum2;
	delete[] modInOtherNum2;

	delete[] modInAmnt2;
	delete[] modInCurve2;

	delete[] modCombineType;
	...
	InstrumentView( _instrument, _parent )
{
	...
	int i = 1;
	connect( tab1Btn, &PixmapButton::clicked, this, [this, i]() { tabBtnClicked(i); } );
	i = 2;
	connect( tab2Btn, &PixmapButton::clicked, this, [this, i]() { tabBtnClicked(i); } );
	i = 3;
	connect( tab3Btn, &PixmapButton::clicked, this, [this, i]() { tabBtnClicked(i); } );
	i = 4;
	connect( tab4Btn, &PixmapButton::clicked, this, [this, i]() { tabBtnClicked(i); } );
	i = 5;
	connect( tab5Btn, &PixmapButton::clicked, this, [this, i]() { tabBtnClicked(i); } );
	i = 6;
	connect( tab6Btn, &PixmapButton::clicked, this, [this, i]() { tabBtnClicked(i); } );


	connect( mainFlipBtn, SIGNAL (clicked () ), this, SLOT ( updateScroll() ) );

	connect( subFlipBtn, SIGNAL (clicked () ), this, SLOT ( updateScroll() ) );
...
void MicrowaveView::updateScroll()
{
	...
	int mainFlipped = castModel<Microwave>()->mainFlipped.value();
	int subFlipped = castModel<Microwave>()->subFlipped.value();
	...
	for( int i = 0; i < 8; ++i )
	{
		...
		sampLenKnob[i]->move( 0 - scrollVal, 50 + mainIsNotFlipped );
		morphMaxKnob[i]->move( 0 - scrollVal, 75 + mainIsNotFlipped );
		...
		detuneKnob[i]->move( 148 - scrollVal, 216 + mainIsFlipped );
		phaseKnob[i]->move( 86 - scrollVal, 216 + mainIsNotFlipped );
		phaseRandKnob[i]->move( 113 - scrollVal, 216 + mainIsNotFlipped );
		volKnob[i]->move( 0 - scrollVal, 100 + mainIsNotFlipped );
		enabledToggle[i]->move( 50 - scrollVal, 240 + mainIsFlipped );
		mutedToggle[i]->move( 0 - scrollVal, 205 + mainIsFlipped );
	...
	for( int i = 0; i < 64; ++i )
	{
		...
		subPhaseKnob[i]->move( 97 + 250 - scrollVal, 172 + subIsFlipped );
		subPhaseRandKnob[i]->move( 129 + 250 - scrollVal, 172 + subIsFlipped );
		subDetuneKnob[i]->move( 60 + 250 - scrollVal, 0 + subIsFlipped );
		subMutedToggle[i]->move( 147 + 250 - scrollVal, 214 + subIsFlipped );
		subKeytrackToggle[i]->move( 108 + 250 - scrollVal, 229 + subIsFlipped );
		subSampLenKnob[i]->move( 187 + 250 - scrollVal, 172 + subIsFlipped );
		subNoiseToggle[i]->move( 147 + 250 - scrollVal, 229 + subIsFlipped );
		subPanningKnob[i]->move( 55 + 250 - scrollVal, 172 + subIsFlipped );
		subTempoKnob[i]->move( 30 + 250 - scrollVal, 0 + subIsNotFlipped );
	...
	macroKnob[0]->move( 1500 + 30 - scrollVal, 100 );
	macroKnob[1]->move( 1500 + 60 - scrollVal, 100 );
	macroKnob[2]->move( 1500 + 90 - scrollVal, 100 );
	macroKnob[3]->move( 1500 + 120 - scrollVal, 100 );
	macroKnob[4]->move( 1500 + 30 - scrollVal, 130 );
	macroKnob[5]->move( 1500 + 60 - scrollVal, 130 );
	macroKnob[6]->move( 1500 + 90 - scrollVal, 130 );
	macroKnob[7]->move( 1500 + 120 - scrollVal, 130 );
	...
	loadModeBox->move( 1750 + 0 - scrollVal, 0 );
...
void MicrowaveView::wheelEvent( QWheelEvent * _me )
{
	if( _me->x() <= 18 && _me->y() >= 48 && _me->y() <= 138 )// If scroll over tab buttons
	{
		if( _me->delta() < 0 )
		{
			castModel<Microwave>()->scroll.setValue( castModel<Microwave>()->scroll.value() + 1 );
		}
		else if( _me->delta() > 0 )
		{
			castModel<Microwave>()->scroll.setValue( castModel<Microwave>()->scroll.value() - 1 );
...
void MicrowaveView::openWavetableFile( QString fileName )
{
	...
	int algorithm = castModel<Microwave>()->loadMode.value();
	int channel = castModel<Microwave>()->loadChnl.value();
	if( !fileName.isEmpty() )
	{
		...
		switch( algorithm )
		{
			...
			case 2:// Autocorrelation
			{
				...
				for( int i = 0; i < 524288; ++i )
				{
					castModel<Microwave>()->waveforms[oscilNum][i] = sampleBuffer->userWaveSample( ((i/2048.f)*period)/lengthOfSample, channel );
				}
...
/*			case 2:// Zero-crossing method
			{
				const int gap = castModel<Microwave>()->wtLoad1.value();
				std::vector<float> crossings;
				int loc = 0;
				...
				for( int j = 0; j < 256; ++j )
				{
					...
					for( int i = 0; i < 2048; ++i )
					{
						castModel<Microwave>()->waveforms[oscilNum][j*2048+i] = sampleBuffer->userWaveSample( ((i/2048.f)*(temp2-temp1)+temp1)/lengthOfSample, channel );
					}
				}
		...
		for( int j = 0; j < 256; ++j )
		{
			start = -castModel<Microwave>()->waveforms[oscilNum][j*2048];
			end = -castModel<Microwave>()->waveforms[oscilNum][j*2048+2047];
			for( int i = 0; i < 2048; ++i )
			{
				castModel<Microwave>()->waveforms[oscilNum][j*2048+i] += (i/2048.f)*end + ((2048.f-i)/2048.f)*start;
			}
		}*/
...
std::vector<float> mSynth::nextStringSample( float (&waveforms)[8][524288], float * subs, float * sampGraphs, std::vector<float> (&samples)[8][2], int maxFiltEnabled, int maxModEnabled, int maxSubEnabled, int maxSampleEnabled, int maxMainEnabled, int sample_rate )
{
	...
	for( int l = 0; l < maxModEnabled; ++l )// maxModEnabled keeps this from looping 64 times every sample, saving a lot of CPU
	{
		if( modEnabled[l] )
		{
			...
			// Calculate curve
			if( modCombineType[l] <= 1 )// Unidirectional
			{
			...
			else// Bidirectional
			{
				if( modInCurve[l] != 100.f )// The "if" statement is there so unnecessary CPU isn't spent (pow is very expensive) if the curve knob isn't being used.
				{
					curModValCurve[0] = ( (curModVal[0] <= -1) ? curModVal[0] : pow( abs( curModVal[0] ), 1.f / ( modInCurve[l] * 0.01f ) ) * ( curModVal[0] < 0 ? -1 : 1 ) ) + (modInAmnt[l] / 200.f);
					curModValCurve[1] = ( (curModVal[1] <= -1) ? curModVal[1] : pow( abs( curModVal[1] ), 1.f / ( modInCurve[l] * 0.01f ) ) * ( curModVal[1] < 0 ? -1 : 1 ) ) + (modInAmnt[l] / 200.f);
				}
				else
				{
					curModValCurve[0] = curModVal[0] + (modInAmnt[l] / 200.f);
					curModValCurve[1] = curModVal[1] + (modInAmnt[l] / 200.f);
				}
				if( modInCurve2[l] != 100.f )
				{
					curModVal2Curve[0] = ( (curModVal2[0] <= -1) ? curModVal[0] : pow( abs( curModVal2[0] ), 1.f / ( modInCurve2[l] * 0.01f ) ) * ( curModVal2[0] < 0 ? -1 : 1 ) ) + (modInAmnt[l] / 200.f);
					curModVal2Curve[1] = ( (curModVal2[1] <= -1) ? curModVal[1] : pow( abs( curModVal2[0] ), 1.f / ( modInCurve2[l] * 0.01f ) ) * ( curModVal2[0] < 0 ? -1 : 1 ) ) + (modInAmnt[l] / 200.f);
				}
				else
				{
					curModVal2Curve[0] = curModVal2[0] + (modInAmnt[l] / 200.f);
					curModVal2Curve[1] = curModVal2[1] + (modInAmnt[l] / 200.f);
				}
			}
			
			switch( modCombineType[l] )
			{
				case 0:// Add Unidirectional
					comboModVal[0] = curModValCurve[0] + curModVal2Curve[0] - 1;
					comboModVal[1] = curModValCurve[1] + curModVal2Curve[1] - 1;
					break;
				case 1:// Multiply Unidirectional
					comboModVal[0] = curModValCurve[0] * curModVal2Curve[0] - 1;
					comboModVal[1] = curModValCurve[1] * curModVal2Curve[1] - 1;
					break;
				case 2:// Add Bidirectional
					comboModVal[0] = ( curModValCurve[0] + curModVal2Curve[0] );
					comboModVal[1] = ( curModValCurve[1] + curModVal2Curve[1] );
					break;
				case 3:// Multiply Bidirectional
					comboModVal[0] = curModValCurve[0] * curModVal2Curve[0];
					comboModVal[1] = curModValCurve[1] * curModVal2Curve[1];
					break;
				default:
	...
	// As much as it may seem like it, this section has no intentional attempts at obfuscation.
		...
		for( int l = 0; l < unisonVoices[i]; ++l )
		{
			...
			switch( modifyModeVal[i] )// More horrifying formulas for the various Modify Modes
			{
				...
				case 14:// Clip
					...
					resultsample[i][l] = qBound( -temp1, resultsample[i][l], temp1 );
					break;
				case 15:// Inverse Clip
					temp1 = (modifyVal[i]/( sampLen[i] - 1 ));
					if( resultsample[i][l] > 0 && resultsample[i][l] < temp1 )
					{
						resultsample[i][l] = temp1;
					}
					else if( resultsample[i][l] < 0 && resultsample[i][l] > -temp1 )
					{
						resultsample[i][l] = -temp1;
					}
					break;
				case 16:// Sine
					temp1 = (modifyVal[i]/( sampLen[i] - 1 ));
					resultsample[i][l] = sin( resultsample[i][l] * F_PI );
					break;
				case 17:// Atan
					temp1 = (modifyVal[i]/( sampLen[i] - 1 ));
					resultsample[i][l] = atan( resultsample[i][l] * F_PI );
					break;
				default:
					{}
			}
	...
	for( int l = 0; l < maxSubEnabled; ++l )// maxSubEnabled keeps this from looping 64 times every sample, saving a lot of CPU
	{
		...
		else
		{
			lastSubVal[l][0] = 0;
			lastSubVal[l][1] = 0;
			lastSubEnvVal[l][0] = subsample[l][0];
			lastSubEnvVal[l][1] = subsample[l][1];
		}
	}
