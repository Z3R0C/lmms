...
private:
	...
	FloatModel  * m_filtDetune[8];// This changes the delay time to change the pitch
	...
	int maxModEnabled = 0;//64;// The highest number of matrix sections that must be looped through
...
protected slots:
	...
	void openWavetableFile( int algorithm, int channel, QString fileName = "" );
...
private:
	...
	Knob * m_filtFeedbackKnob[8];
	Knob * m_filtDetuneKnob[8];
	...
	Knob * m_modifyKnob[8];
	ComboBox * m_modOutSecBox[64];
	ComboBox * m_modOutSigBox[64];
	LcdSpinBox * m_modOutSecNumBox[64];
	ComboBox * m_modInBox[64];
	LcdSpinBox * m_modInNumBox[64];
	LcdSpinBox * m_modInOtherNumBox[64];
	Knob * m_modInAmntKnob[64];
	Knob * m_modInCurveKnob[64];
	ComboBox * m_modInBox2[64];
	LcdSpinBox * m_modInNumBox2[64];
	LcdSpinBox * m_modInOtherNumBox2[64];
	Knob * m_modInAmntKnob2[64];
	Knob * m_modInCurveKnob2[64];
	LedCheckBox * m_modEnabledToggle[64];
	ComboBox * m_modCombineTypeBox[64];
	...
	Knob * m_sampleStartKnob[8];
	Knob * m_sampleEndKnob[8];
...
public:
	mSynth( NotePlayHandle * _nph,
			...
			float * phaseRand,
			int * modifyModeVal, float * _modifyVal, float * _morphVal, float * rangeVal, float * sprayVal, float * unisonVoices, float * unisonDetune, float * unisonMorph, float * unisonModify, float * morphMaxVal, float * detuneVal, float waveforms[8][524288], float * subs, bool * subEnabled, float * subVol, float * subPhase, float * subPhaseRand, float * subDetune, bool * subMuted, bool * subKeytrack, float * subSampLen, bool * subNoise, int * sampLen, int * m_modIn, int * m_modInNum, int * m_modInOtherNum, float * m_modInAmnt, float * m_modInCurve, int * m_modIn2, int * m_modInNum2, int * m_modInOtherNum2, float * m_modInAmnt2, float * m_modInCurve2, int * m_modOutSec, int * m_modOutSig, int * m_modOutSecNum, int * m_modCombineType, float * phase, float * vol, float * filtInVol, int * filtType, int * filtSlope, float * filtCutoff, float * filtReso, float * filtGain, float * filtSatu, float * filtWetDry, float * filtBal, float * filtOutVol, bool * filtEnabled, bool * enabled, bool * modEnabled, float * sampGraphs, bool * muted, bool * sampleEnabled, bool * sampleGraphEnabled, bool * sampleMuted, bool * sampleKeytracking, bool * sampleLoop, float * sampleVolume, float * samplePanning, float * sampleDetune, float * samplePhase, float * samplePhaseRand, std::vector<float> (&samples)[8][2], float * filtFeedback, float * filtDetune, bool * filtKeytracking, float * subPanning, float * sampleStart, float * sampleEnd );
	virtual ~mSynth();
	
	std::vector<float> nextStringSample( float waveforms[8][524288], float * subs, float * sampGraphs, std::vector<float> (&samples)[8][2], int maxModEnabled, int maxSubEnabled, int maxSampleEnabled, int sample_rate );
...
private:
	...
	std::vector<float> filtDelayBuf[8][8][2];// [filter number][port][channel]
	...
	int m_modIn[64];
	int m_modInNum[64];
	int m_modInOtherNum[64];
	float m_modInAmnt[64];
	float m_modInCurve[64];
	int m_modIn2[64];
	int m_modInNum2[64];
	int m_modInOtherNum2[64];
